...
void Baja()
{
	do
	{
		printf("Digite el numero de registros que desea dar de baja: \n");
		scanf_s("%d", &baja);
	...
	numarticulo = new int[baja]; //arreglos
	subtotal = new float[baja];
	genero = new string[baja];
	consola = new string[baja];
	desc = new string[baja];
	nombre = new string[baja];
	clasif = new string[baja];
	res = new string[baja];
	status = new string[baja];
	...
	for (int i = 0; i < baja; i++)
	{
		...
		printf("-El precio del producto en baja es de: $ %f\n", subtotal[i]);
		status[i] = "ACTIVO";
	}
}
